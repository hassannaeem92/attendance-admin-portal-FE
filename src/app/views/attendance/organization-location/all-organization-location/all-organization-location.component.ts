...
@Component({
  selector: 'app-all-organization-location',
  standalone: true,
  imports: [
    MatButtonModule,
    RouterLink,
  ...
  templateUrl: './all-organization-location.component.html',
  styleUrl: './all-organization-location.component.scss'
})
export class AllOrganizationLocationComponent {
readonly utils = inject(Utils);
 ...
 messages = {
  ...
  isOrganizationLocation: boolean = true;
  ...
  setPageAdmin(pageInfo: any, flag?: any) {
    ...
    this.subscription = this.locationService.getAllLocationsForAdmin(this.page, pIndex, this.orgIdFromLocalHost,this.isActive).subscribe((pagedData) => {
      if (pagedData) {
        this.page.totalElements = pagedData.TotalPages;
        // this.page.totalElements = this.page.totalElements;
        var filteredRows = pagedData.Result;
  ...
  edit(event: any) {
    ...
    if (event && event.cellIndex !== 0) {
      if (event.type === 'click' && event.type != 'checked') {
        this.router.navigate(['/main/attendance/company-location/edit', event.row.Id]);
      }
    }
  }
  ...
  //         this.utils.notification('Location Active Successfully.', 'success');
  //         this.setPage({ offset: 0 });
  //         this.utils.hideLoader();
  //       }
  //     })
  ...
  //       if (data.StatusCode == 200) {
  //         // Utils.notification(data.btiMessage.message, 'success');
  //         this.utils.notification('Location InAcitve Successfully.', 'success');
  //         this.setPage({ offset: 0 });
    ...
    if (row.IsActive) {
      ...
      this.utils.confirmDelete().then((result: boolean) => {
        if (result) {
          ...
          this.locationService.deleteLocation(body).subscribe(data => {
            if (data.StatusCode === 200) {
              this.utils.notification('Location Inactive Successfully.', 'success');
            } else {
              this.utils.notification(data.CommonMessage || 'Failed to deactivate location.', 'error');
              this.setPage({ offset: 0 });
            }
            this.setPage({ offset: 0 });
            this.utils.hideLoader();
      ...
      this.locationService.addLocation(row).subscribe(res => {
        if (res && res.StatusCode === 200) {
          this.utils.notification('Location Activated Successfully.', 'success');
          this.setPage({ offset: 0 });
        } else {
          this.utils.notification('Failed to activate location.', 'error');
        }
        this.utils.hideLoader();
      });
    }
