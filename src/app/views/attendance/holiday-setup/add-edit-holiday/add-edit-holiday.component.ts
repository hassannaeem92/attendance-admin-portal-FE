...
@Component({
  ...
  styleUrl: './add-edit-holiday.component.scss'
})
...
readonly utils = inject(Utils);
    ...
    onSubmit(NgForm: any) {
      ...
      const body  = {
          ...
          Date: this.shiftDateValue ? this.shiftDateValue : '',
      ...
      this.holidayService.addEditHolidays(body).subscribe(res => {
        if (res && res.StatusCode == 200) {
          this.utils.notification(res.CommonMessage, 'success');
          this.router.navigate(['/main/attendance/holiday']);
    ...
    onDateChange(event){
      this.shiftDateValue = this.dateService.formatDate(event.target.value, 'yyyy-MM-dd');
    }
    ...
    getAllShift(id, fromEdit?){
      ...
      this.shiftAssignService.getAllShiftsDrop(orgId, this.selectedCompany?.id).subscribe(res => {
        ...
        if(this.shiftArray && fromEdit && this.mode == 'edit' && this.rowsData.ShiftId){
    ...
    getById(id: any){
      ...
      this.holidayService.getHolidaysById(body).subscribe(res => {
        if (res && res.StatusCode == 200) {
           this.rowsData = res.Result;
          
          
        if(this.rowsData.Date){
          this.shiftDate = new Date(this.rowsData.Date);
          this.shiftDateValue = this.dateService.formatDate(this.shiftDate, 'yyyy-MM-dd');
        }
