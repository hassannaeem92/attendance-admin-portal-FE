...
@Component({
  ...
  styleUrl: './all-shift.component.scss'
})
export class AllShiftComponent {
readonly utils = inject(Utils);
 ...
 messages = {
  ...
  setPage(pageInfo: any, flag?: any) {
    ...
    this.subscription = this.shiftService.getAllShift(this.page, pIndex, this.orgIdFromLocalHost,this.isActive,this.isOrganization).subscribe((pagedData) => {
      if (pagedData) {
  ...
  edit(event: any) {
    ...
    if (event && event.cellIndex !== 0) {
      if (event.type === 'click' && event.type != 'checked') {
        this.router.navigate(['/main/attendance/shift/edit', event.row.Id]);
      }
    }
  }
  ...
  toDelete() {
    ...
    this.shiftService.deleteShift(body).subscribe(data => {
  ...
  //   {
  //     row.IsActive = true;
  //     this.shiftService.addEditShift(row).subscribe(res => {
  //       if (res && res.StatusCode == 200) {
  //         this.utils.notification('Shift Active Successfully.', 'success');
  //         this.setPage({ offset: 0 });
  //         this.utils.hideLoader();
  //       }
  //     })
  ...
  //       if (data.StatusCode == 200) {
  //         // Utils.notification(data.btiMessage.message, 'success');
  //         this.utils.notification('Shift InAcitve Successfully.', 'success');
  //         this.setPage({ offset: 0 });
    ...
    if (row.IsActive) {
      ...
      this.utils.confirmDelete().then((result: boolean) => {
        if (result) {
          ...
          this.shiftService.deleteShift(body).subscribe(data => {
            if (data.StatusCode === 200) {
              this.utils.notification('Shift Inactive Successfully.', 'success');
            } else {
              this.utils.notification(data.CommonMessage || 'Failed to update shift status.', 'error');
            }
            this.setPage({ offset: 0 });
            this.utils.hideLoader();
          });
      ...
      this.shiftService.addEditShift(row).subscribe(res => {
        if (res && res.StatusCode === 200) {
          this.utils.notification('Shift Activated Successfully.', 'success');
        } else {
          this.utils.notification('Failed to activate shift.', 'error');
        }
        this.setPage({ offset: 0 });
        this.utils.hideLoader();
      });
