...
@Component({
  selector: 'app-raw-attendance-report',
  standalone: true,
  imports: [
    MatButtonModule,
    RouterLink,
  ...
  templateUrl: './raw-attendance-report.component.html',
  styleUrl: './raw-attendance-report.component.scss'
})
export class RawAttendanceReportComponent {
readonly utils = inject(Utils);
 ...
 messages = {
  ...
  setPage(pageInfo: any, flag?: any) {
    ...
    this.subscription = this.deviceService.getAllRawAttendacneData(body).subscribe((pagedData) => {
      if (pagedData && pagedData.Result && pagedData.Result.length > 0) {
