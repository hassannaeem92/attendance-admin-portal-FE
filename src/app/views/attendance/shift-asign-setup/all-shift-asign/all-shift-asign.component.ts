...
import { ShiftAssignUserModalComponent } from "../../../../_sharedresources/modals/shift-assign-user-modal/shift-assign-user-modal.component";
import { MatIconModule } from '@angular/material/icon';
declare const $: any;


@Component({
  selector: 'app-all-shift-asign',
  standalone: true,
  imports: [
    MatButtonModule,
    RouterLink,
    ...
    ShiftAssignUserModalComponent,
    MatIconModule 
],
  templateUrl: './all-shift-asign.component.html',
  styleUrl: './all-shift-asign.component.scss'
})
export class AllShiftAsignComponent {
readonly utils = inject(Utils);
 ...
 messages = {
  ...
  setPage(pageInfo: any, flag?: any) {
    ...
    this.subscription = this.shiftAssignService.getAllShiftAssign(this.page, pIndex, this.orgIdFromLocalHost,this.isActive,this.isOrganization).subscribe((pagedData) => {
      if (pagedData) {
  ...
  edit(event: any) {
    ...
    if (event && event.cellIndex !== 0) {
      if (event.type === 'click' && event.type != 'checked') {
        this.router.navigate(['/main/attendance/shift-assign/edit', event.row.Id]);
      }
    }
  }
  ...
  toDelete() {
    ...
    this.shiftAssignService.deleteShiftAssign(body).subscribe(data => {
  ...
  //   {
  //     row.IsActive = true;
  //     this.shiftAssignService.addEditShiftAssign(row).subscribe(res => {
  //       if (res && res.StatusCode == 200) {
  //         this.utils.notification('Shift Assign Active Successfully.', 'success');
  //         this.setPage({ offset: 0 });
  //         this.utils.hideLoader();
  //       }
  //     })
  ...
  //       if (data.StatusCode == 200) {
  //         // Utils.notification(data.btiMessage.message, 'success');
  //         this.utils.notification('Shift Assign InAcitve Successfully.', 'success');
  //         this.setPage({ offset: 0 });
    ...
    if (row.IsActive) {
      ...
      this.utils.confirmDelete().then((result: boolean) => {
        if (result) {
          ...
          this.shiftAssignService.deleteShiftAssign(body).subscribe(data => {
            if (data.StatusCode === 200) {
              this.utils.notification('Shift Assignment Inactive Successfully.', 'success');
            } else {
              this.utils.notification(data.CommonMessage || 'Failed to update shift assignment status.', 'error');
            }
            this.setPage({ offset: 0 });
            this.utils.hideLoader();
          });
      ...
      this.shiftAssignService.addEditShiftAssign(row).subscribe(res => {
        if (res && res.StatusCode === 200) {
          this.utils.notification('Shift Assignment Activated Successfully.', 'success');
        } else {
          this.utils.notification('Failed to activate shift assignment.', 'error');
        }
        this.setPage({ offset: 0 });
        this.utils.hideLoader();
      });
  ...
  shiftAssignArrayData: any = [];
  openShiftAssignModal(row, event){
    event.stopPropagation();
    
    if(row && row.AssignUsers && row.AssignUsers.length > 0){
      this.shiftAssignArrayData = row.AssignUsers;
      $('#shiftAssign-modal').modal('show');
    }else {
      this.utils.notification('No User Assign', 'error');
    }
