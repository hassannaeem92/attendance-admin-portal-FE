...
export class AllDevicesComponent {
 ...
 messages = {
  ...
  setPage(pageInfo: any, flag?: any) {
    ...
    this.subscription = this.deviceService.getAllDevice(this.page, pIndex, this.orgIdFromLocalHost,this.isActive,this.isOrganization).subscribe((pagedData) => {
      if (pagedData) {
    ...
    if (row.IsActive) {
      ...
      this.utils.confirmDelete().then((result: boolean) => {
        if (result) {
          ...
          this.deviceService.deleteDevice(body).subscribe(data => {
            if (data.StatusCode === 200) {
              this.utils.notification('Device Inactive Successfully.', 'success');
            } else {
              this.utils.notification(data.CommonMessage || 'Failed to update device status.', 'error');
            }
            this.setPage({ offset: 0 });
            this.utils.hideLoader();
          });
      ...
      this.deviceService.addEditDevice(row).subscribe(res => {
        if (res && res.StatusCode === 200) {
          this.utils.notification('Device Activated Successfully.', 'success');
        } else {
          this.utils.notification('Failed to activate device.', 'error');
        }
        this.setPage({ offset: 0 });
        this.utils.hideLoader();
      });
