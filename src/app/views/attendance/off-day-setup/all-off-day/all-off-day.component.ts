...
@Component({
  ...
  templateUrl: './all-off-day.component.html',
  styleUrl: './all-off-day.component.scss'
})
export class AllOffDayComponent {
readonly utils = inject(Utils);
 ...
 messages = {
  ...
  setPage(pageInfo: any, flag?: any) {
    ...
    this.subscription = this.offDayService.getAllOffDay(this.page, pIndex, this.orgIdFromLocalHost,this.isActive,this.isOrganization).subscribe((pagedData) => {
      if (pagedData) {
  ...
  edit(event: any) {
    ...
    if (event && event.cellIndex !== 0) {
      if (event.type === 'click' && event.type != 'checked') {
        this.router.navigate(['/main/attendance/off-day/edit', event.row.Id]);
      }
    }
  }
  ...
  toDelete() {
    ...
    this.offDayService.deleteOffDay(body).subscribe(data => {
  ...
  //   {
  //     row.IsActive = true;
  //     this.offDayService.addEditOffDay(row).subscribe(res => {
  //       if (res && res.StatusCode == 200) {
  //         this.utils.notification('OffDay Active Successfully.', 'success');
  //         this.setPage({ offset: 0 });
  //         this.utils.hideLoader();
  //       }
  //     })
  ...
  //       if (data.StatusCode == 200) {
  //         // Utils.notification(data.btiMessage.message, 'success');
  //         this.utils.notification('OffDay InAcitve Successfully.', 'success');
  //         this.setPage({ offset: 0 });
    ...
    if (row.IsActive) {
      ...
      this.utils.confirmDelete().then((result: boolean) => {
        if (result) {
          ...
          this.offDayService.deleteOffDay(body).subscribe(data => {
            if (data.StatusCode === 200) {
              this.utils.notification('OffDay Inactive Successfully.', 'success');
            } else {
              this.utils.notification(data.CommonMessage || 'Failed to update OffDay status.', 'error');
            }
            this.setPage({ offset: 0 });
            this.utils.hideLoader();
          });
      ...
      this.offDayService.addEditOffDay(row).subscribe(res => {
        if (res && res.StatusCode === 200) {
          this.utils.notification('OffDay Activated Successfully.', 'success');
        } else {
          this.utils.notification('Failed to activate OffDay.', 'error');
        }
        this.setPage({ offset: 0 });
        this.utils.hideLoader();
      });
