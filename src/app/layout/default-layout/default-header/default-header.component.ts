...
export class DefaultHeaderComponent extends HeaderComponent {
  ...
  profileImageUrl: any = '/assets/images/no-profile-pic-tiny.png';
  currentUserId: any;
  ...
  ngOnInit() {
    ...
    this.userService.getUserById(body).subscribe((data: any) => {
      var result  = data.Result;
      
      // if (result.ImageName && result.Image) {
      if (result && typeof result.Image === 'string' && result.Image.trim() !== '') {

        this.profileImageUrl = environment.userModuleApiImageBaseUrl + result.Image
      } else {
        this.profileImageUrl = '/assets/images/no-profile-pic-tiny.png'; // Use fallback
      }
    })
  }
  ...
  constructor() {
    ...
    this.themeService.setTheme(c);

  }


  colorModeChange(theme: any) {
    this.themeService.setTheme(theme);
  }

  sidebarCollapsed = false; // To track whether the sidebar is collapsed (icon-only) or expanded (full)

  toggleSidebarCollapse() {
    this.sidebarCollapsed = !this.sidebarCollapsed; // Toggle collapse/expand state
  }
  toggleSidebar() {
  ...
  logout() {
    ...
    this.utils.notification('Logout Successfully', 'success');
