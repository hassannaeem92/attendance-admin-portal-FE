...
@Component({
  selector: 'app-organization-location-modal',
  standalone: true,
  ...
  templateUrl: './organization-location-modal.component.html',
  styleUrl: './organization-location-modal.component.scss'
})
export class OrganizationLocationModalComponent {

  @Output() isOrgLocAdded = new EventEmitter<any>();
  ...
  onSubmit(NgForm: any) {
    ...
    this.locationService.addLocation(body).subscribe(res => {
      if (res && res.StatusCode == 200) {
        ...
        $('#organization-location-modal').modal('hide');

        this.isOrgLocAdded.emit(true);
