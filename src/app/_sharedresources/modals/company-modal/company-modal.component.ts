...
export class CompanyModalComponent {
  @Output() isComAdded = new EventEmitter<any>();
 ...
 intervalId: any
    ...
    const body = {
     ...
     this.companyService.addCompany(body).subscribe(res => {
       if (res && res.StatusCode == 200) {
         ...
         this.isComAdded.emit(true);
          $('#company-modal').modal('hide');
        //  this.router.navigate(['/main/attendance/company/all-company']);
   ...
   onLogoUpload(event: Event): void {
     ...
     if (input.files && input.files[0]) {
       ...
       reader.onload = () => {
         this.logoPreview = reader.result; // For preview
       };
